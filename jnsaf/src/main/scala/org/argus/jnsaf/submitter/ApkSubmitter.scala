...
object ApkSubmitter {
  def apply(sourcePath: String, address: String, port: Int): IMap[String, Option[TaintResult]] = {
    val analysisResult: MMap[String, Option[TaintResult]] = mmapEmpty
    val fileOrDir = new File(sourcePath)
    fileOrDir match {
      case dir if dir.isDirectory =>
        submitApkInDir(FileUtil.toUri(dir), address, port, analysisResult)
      case file =>
        if(ApkGlobal.isValidApk(FileUtil.toUri(file)))
          submitApk(FileUtil.toUri(file), address, port, analysisResult)
        else println(file + " is not decompilable.")
    }
    analysisResult.toMap
  }
  def submitApkInDir(dirUri: FileResourceUri, address: String, port: Int, result: MMap[String, Option[TaintResult]]): Unit = {
    FileUtil.listFiles(dirUri, ".apk", recursive = true) foreach { apkUri =>
      submitApk(apkUri, address, port, result)
    }
  }
  def submitApk(apkUri: FileResourceUri, address: String, port: Int, result: MMap[String, Option[TaintResult]]): Unit = {
    ...
    result(apk.getName) = client.taintAnalysis(apkUri)
  }
}
