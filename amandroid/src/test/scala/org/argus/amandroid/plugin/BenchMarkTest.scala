...
class BenchMarkTest extends FlatSpec with Matchers {
  ...
  private def taintAnalysis(apkFiles: Set[String]): Option[TaintAnalysisResult] = {
    ...
    val res = TaintAnalysisTask(TaintAnalysisModules.DATA_LEAKAGE, fileUris.map((_, outputUri)), forceDelete = true, reporter, guessPackage = true, TaintAnalysisApproach.COMPONENT_BASED).run
    if(!DEBUG) {
