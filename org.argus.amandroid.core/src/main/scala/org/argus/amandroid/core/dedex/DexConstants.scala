...
trait DexConstants {
  ...
  def const(x: String, y: Int, typ: JawaType, typstr: String): String = {
    typ match {
  ...
  def constWide(x: String, y: Long, typ: JawaType): String = {
    typ match {
      case pt if pt.isPrimitive =>
        pt.jawaName match {
          case "int" => "%s:= %dI;".format(x, y)
...
//  def filledNewArrayRange(baseTyp: String, regbase: Int, regsize: Int) = s"temp:= new $baseTyp[${(0 to regsize - 1).map(i => "v" + (regbase + i)).mkString(", ")}];" // 0x25
  ...
  def ifNq(x: String, y: String, target: Long): String = "if %s != %s then goto L%06x;".format(x, y, target) // 0x33
