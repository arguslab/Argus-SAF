...
import org.objectweb.asm.{Handle, Label, Opcodes}

class BytecodeInstructions(val sig: Signature, pos: Position) {
  ...
  private val instructions: MList[Instruction] = mlistEmpty

  def addInsn(instruction: Instruction): Unit = {
    instructions += instruction
  }

  def getInstructions: IList[Instruction] = instructions.toList
  ...
  case class Insn(opcode: Int) extends Instruction {
    private def xaload(typ: JawaType): Statement = {
      val idx = pop
      val base = pop
      val temp = push(typ)
      val ie = new IndexingExpression(base, List(idx))
      new AssignmentStatement(temp, ie, ilistEmpty)
    }

    private def xstore(isObject: Boolean): Statement = {
      val value = pop
      val idx = pop
      val base = pop
      val ie = new IndexingExpression(base, List(idx))
      val anno: IList[Annotation] = if(isObject) {
        List(objectAnnotation)
      } else {
        ilistEmpty
      }
      new AssignmentStatement(ie, value, anno)
    }

    private def binary(typ: JawaType, op: String): Statement = {
      val r = pop
      val l = pop
      val be = new BinaryExpression(l, op, r)
      val temp = push(typ)
      new AssignmentStatement(temp, be, ilistEmpty)
    }

    private def unary(typ: JawaType, op: String): Statement = {
      val ue = new UnaryExpression(op, pop)
      val temp = push(typ)
      new AssignmentStatement(temp, ue, ilistEmpty)
    }

    private def cast(typ: JawaType): Statement = {
      val v = pop
      val temp = push(typ)
      val ce = new CastExpression(typ, v)
      new AssignmentStatement(temp, ce, ilistEmpty)
    }

    private def cmp(k: String): Statement = {
      val r = pop
      val l = pop
      val temp = push(JavaKnowledge.INT)
      val ce = new CmpExpression(k, l, r)
      new AssignmentStatement(temp, ce, ilistEmpty)
    }
    ...
    def exec(): Unit = {
      ...
      opcode match {
        ...
        case IALOAD =>
          stmt = Some(xaload(JavaKnowledge.INT))
        case LALOAD =>
          stmt = Some(xaload(JavaKnowledge.LONG))
        case FALOAD =>
          stmt = Some(xaload(JavaKnowledge.FLOAT))
        case DALOAD =>
          stmt = Some(xaload(JavaKnowledge.DOUBLE))
        case BALOAD =>
          stmt = Some(xaload(JavaKnowledge.BOOLEAN))
        case CALOAD =>
          stmt = Some(xaload(JavaKnowledge.CHAR))
        case SALOAD =>
          stmt = Some(xaload(JavaKnowledge.SHORT))
        case IASTORE =>
          stmt = Some(xstore(false))
        case LASTORE =>
          stmt = Some(xstore(false))
        case FASTORE =>
          stmt = Some(xstore(false))
        ...
        case SWAP =>
          swap()
        case IADD =>
          stmt = Some(binary(JavaKnowledge.INT, "+"))
        case LADD =>
          stmt = Some(binary(JavaKnowledge.LONG, "+"))
        case FADD =>
          stmt = Some(binary(JavaKnowledge.FLOAT, "+"))
        case DADD =>
          stmt = Some(binary(JavaKnowledge.DOUBLE, "+"))
        case ISUB =>
          stmt = Some(binary(JavaKnowledge.INT, "-"))
        case LSUB =>
          stmt = Some(binary(JavaKnowledge.LONG, "-"))
        ...
        case DREM =>
          stmt = Some(binary(JavaKnowledge.DOUBLE, "%%"))
        case INEG =>
          stmt = Some(unary(JavaKnowledge.INT, "-"))
        case LNEG =>
          stmt = Some(unary(JavaKnowledge.LONG, "-"))
        case FNEG =>
          stmt = Some(unary(JavaKnowledge.FLOAT, "-"))
        case DNEG =>
          stmt = Some(unary(JavaKnowledge.DOUBLE, "-"))
        case ISHL =>
          stmt = Some(binary(JavaKnowledge.INT, "^<"))
        case LSHL =>
          stmt = Some(binary(JavaKnowledge.LONG, "^<"))
        ...
        case LXOR =>
          stmt = Some(binary(JavaKnowledge.LONG, "^~"))
        case I2L =>
          stmt = Some(cast(JavaKnowledge.LONG))
        case I2F =>
          stmt = Some(cast(JavaKnowledge.FLOAT))
        case I2D =>
          stmt = Some(cast(JavaKnowledge.DOUBLE))
        case L2I =>
          stmt = Some(cast(JavaKnowledge.INT))
        case L2F =>
          stmt = Some(cast(JavaKnowledge.FLOAT))
        case L2D =>
          stmt = Some(cast(JavaKnowledge.DOUBLE))
        ...
        case D2F =>
          stmt = Some(cast(JavaKnowledge.FLOAT))
        case I2B =>
          stmt = Some(cast(JavaKnowledge.BYTE))
        case I2C =>
          stmt = Some(cast(JavaKnowledge.CHAR))
        case I2S =>
          stmt = Some(cast(JavaKnowledge.SHORT))
        case LCMP =>
          stmt = Some(cmp("lcmp"))
        case FCMPL =>
          stmt = Some(cmp("fcmpl"))
        ...
        case DCMPG =>
          stmt = Some(cmp("dcmpg"))
        case IRETURN =>
          stmt = Some(new ReturnStatement(pop))
        case LRETURN =>
          stmt = Some(new ReturnStatement(pop))
        case FRETURN =>
          stmt = Some(new ReturnStatement(pop))
        case DRETURN =>
          stmt = Some(new ReturnStatement(pop))
        case ARETURN =>
          stmt = Some(new ReturnStatement(pop, List(objectAnnotation)))
        case RETURN =>
          stmt = Some(new ReturnStatement())
        case ARRAYLENGTH =>
          val len = new LengthExpression(pop)
          val temp = push(JavaKnowledge.INT)
          stmt = Some(new AssignmentStatement(temp, len, ilistEmpty))
        case ATHROW =>
          stmt = Some(new ThrowStatement(pop))
        case MONITORENTER =>
          stmt = Some(new MonitorStatement("monitorenter", pop))
        case MONITOREXIT =>
          stmt = Some(new MonitorStatement("monitorexit", pop))
  ...
  /**
    ...
    */
  case class TryCatchBlock(start: Label, end: Label, handler: Label, t: String) extends Instruction {
    def exec(): Unit = {
